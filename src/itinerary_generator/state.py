...
# Coordinate finder agent output model

class CoordinateFinderResult(TypedDict):
    """Result from the coordinate finder agent."""
    attractions_found: List[str]  # names of all attractions with coordinates


# Day organizer agent output model
...
class GraphState(TypedDict):
    ...
    # Intermediate classification results (from classify_attractions tool)
    attraction_classifications: Annotated[Dict[str, Any], operator.or_]  # {"Eiffel Tower": {"type": "preference", "day": 2}, ...}

    # Route optimization config (from configure_route_optimization tool)
    route_optimization_config: Annotated[Dict[str, Any], operator.or_]  # {"enabled": True, "starting_points": {...}, "ending_points": {...}}

    # Day constraints config (from configure_day_constraints tool)
    day_constraints_config: Annotated[Dict[str, Any], operator.or_]  # {"min_per_day": int|None, "max_per_day": int|None}

    # Exclusive days (closed to flexible attraction additions)
    exclusive_days: List[int]  # Days that should NOT receive flexible attractions from K-means
